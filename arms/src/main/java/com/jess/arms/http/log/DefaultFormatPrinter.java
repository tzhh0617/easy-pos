...
public class DefaultFormatPrinter implements FormatPrinter {
    ...
    /**
     * 对 {@code lines} 中的信息进行逐行打印 并去除多行log title
     *
     * @param tag
     * @param lines
     * @param withLineSize 为 {@code true} 时, 每行的信息长度不会超过110, 超过则自动换行
     */
    private static void logLinesNoTag(String tag, String[] lines, boolean withLineSize) {
        ...
        for (String line : lines) {
            ...
            for (int i = 0; i <= lineLength / maxLongSize; i++) {
                ...
                printInfo.append(line.substring(start, end) + "\n");
            }
        }
        LogUtils.debugInfo(resolveTag(tag), DEFAULT_LINE + printInfo);
    }
    ...
    public void printJsonRequest(@NonNull Request request, @NonNull String bodyString) {
        ...
        logLinesNoTag(tag, getRequest(request), true);
        logLinesNoTag(tag, requestBody.split(LINE_SEPARATOR), true);
    ...
    public void printJsonResponse(long chainMs, boolean isSuccessful, int code, @NonNull String headers, @Nullable MediaType contentType,
        ...
        logLinesNoTag(tag, getResponse(headers, chainMs, code, isSuccessful, segments, message), true);
        logLinesNoTag(tag, responseBody.split(LINE_SEPARATOR), true);
        LogUtils.debugInfo(tag, END_LINE);
    }
