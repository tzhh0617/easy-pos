...
import com.jess.arms.widget.lsnackbar.LSnackBar;
import com.jess.arms.widget.lsnackbar.ScreenUtil;
...
public final class AppManager {
    ...
    public void showSnackbar(String message, boolean isLong) {
        ...
        showSnackbar(message,isLong, ScreenUtil.dp2px(context,200),
     ...
     * @param isLong
     */
    public void showSnackbar(String message, boolean isLong,int width,int height) {
        if (getCurrentActivity() == null && getTopActivity() == null) {
            Timber.tag(TAG).w("mCurrentActivity == null when showSnackbar(String,boolean)");
            return;
        }
        Completable.fromAction(() -> {
            ...
            if (DEPENDENCY_SUPPORT_DESIGN) {
                ...
                if (view instanceof ViewGroup) {
                    LSnackBar lSnackBar = LSnackBar.make(view,message, isLong ? LSnackBar.LENGTH_LONG : LSnackBar.LENGTH_SHORT);
                    View lSnackbarView = lSnackBar.getView();
                    lSnackbarView.setPadding(0,0,0,0);
                    TextView tv = lSnackbarView.findViewById(R.id.snackbar_text);
                    tv.setGravity(Gravity.CENTER);
                    tv.setMinHeight(height);
                    tv.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
                    //设置布局居中
                    FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(lSnackbarView.getLayoutParams().width, lSnackbarView.getLayoutParams().height);
                    params.gravity = Gravity.CENTER;
                    params.width = width;
                    params.height = ViewGroup.LayoutParams.WRAP_CONTENT;
                    params.setMargins(0,0,0,0);
                    lSnackbarView.setLayoutParams(params);
                    lSnackbarView.setBackgroundResource(R.drawable.bg_toast);
                    lSnackBar.show();
                    return;
                }
                ...
                TextView tv = snackbarView.findViewById(com.google.android.material.R.id.snackbar_text);
//                com.google.android.material.R.layout.design_layout_snackbar
                tv.setGravity(Gravity.CENTER);
                tv.setHeight(height);
                tv.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
